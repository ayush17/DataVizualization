function loadChoropleth() {
  ...
  var legendsvg = svg
    ...
    .attr(
      "transform",
      "translate(" + svgW / 2 + "," + (svgH - 3 * padding) + ")"
    );
  ...
  // Set metric scale
  var xScale = d3.scaleLinear().range([0, legendWidth]);
  ...
  function updateGradientScale() {
    ...
    gX.transition("gradient")
      ...
      .ease(defaultEase)
      .call(xAxis);
    legendsvg.select(".legendTitle").text(focusMetric);
  }
  ...
  clip.selectAll("path").call(dragGlobe);
  ...
  function rotateGlobe(d) {
    ...
    if (d) {
      ...
      // Sometimes country is not known, path is empty, or no terrorism is present at all
    ...
    (function zoom() {
      clip
        ...
        .tween("transform", function () {
          var targetScale =
            projection.scale() > radius ? radius : zoomFactor * radius;
    ...
    .html(function (p) {
      ...
      if (focusDatasetById.hasOwnProperty(p.id)) {
        ...
        return (
          ...
          p.properties.name +
          "</span><br><br><hr>" +
          Math.round((getMetric(d) / metricSum()) * 10000) / 100 +
          "%<br><br>(" +
          getMetric(d) +
          " out of " +
          metricSum() +
        ...
        return (
          "<span style='color:" +
          d3.color(cScale(0)).brighter(0.5) +
          "'>" +
          p.properties.name +
          "</span><br><br><hr>0%<br><br>(0 out of " +
          metricSum() +
          ")"
        );
      }
  ...
  function setTitle() {
    ...
    tmp.innerHTML =
      "Geographical Distribution by <tspan style='font-style: italic;'>" +
      focusCategory +
      ": " +
      focusSubcategory +
  ...
  var $select1 = $("#choropleth-select-category").selectize({
    options: toOptionsFormat(
      ["All Types", "Weapon Type", "Attack Type", "Target Type"],
      "Type"
  ...
  function startTimer() {
    ...
    timeMachineTimer = d3.interval(function () {
      if (i < yearsToGo.length) {
        ...
        // There is no timer.onstop method
        stopTimer();
      }
    }, defaultDuration);
  }
...
function loadData(callback) {
  d3.queue()
    ...
    .await(function (error, stringsDict, choroplethData, pointsData, worldMap) {
